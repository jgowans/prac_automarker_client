    ...
    def build(self):
        ...
        try:
            assembler_cmdline = ['arm-none-eabi-as',
                                 ...
                                 '-mcpu=cortex-m0',
                                 '-o', 'main.o',
                                 '{s}'.format(self.group.source_file)]
            subprocess.check_output(
                assembler_cmdline, 
                stderr=subprocess.STDOUT)
            logger.info("Assembler ran successfully")
            ...
            subprocess.check_output(
                linkerr_cmdline, 
                stderr=subprocess.STDOUT)
            logger.info("Linker process ran successfully")
        except subprocess.CalledProcessError as e:
            logger.info("Received build error: \n{err}".format(err = e.output))
            raise e
