...
class Prac4Tests:
    def __init__(self, comment, full_path_to_src, submission_dir):
        self.comment = comment
        self.full_path_to_src = full_path_to_src
        self.full_path_to_elf = None
        self.submission_dir = submission_dir

    def build(self):
        as_proc = subprocess.Popen(["arm-none-eabi-as", "-mcpu=cortex-m0", "-mthumb", "-g", "-o", self.submission_dir + "/main.o", self.full_path_to_src], \
        ...
        ld_proc = subprocess.Popen(["arm-none-eabi-ld", "-Ttext=0x08000000", "-o", self.submission_dir + "/main.elf", self.submission_dir + "/main.o"], \
                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if (ld_proc.wait() != 0):
    ...
    def part2_tests(self):
        ...
        self.comment("Setting address 0x20000008 to 0x32897788")
        self.comment("This implies a max unsigned of 0x99, min unsigned of 0x32 and max signed of 0x77")
        self.gdb.write_word(0x20000008, 0x32997788)
        return 2
    ...
    def part3_tests(self):
        ...
        self.comment("Data on LEDs should be max unsigned (0x99), and found to be {d:#X}".format(d=led_data))
        if led_data != 0x99:
    ...
    def part4_tests(self):
        ...
        led_data = self.ii.read_port(0)
        self.comment("Data on LEDs should be min unsigned (0x32), and found to be {d:#X}".format(d=led_data))
        if led_data != 0x32:
    ...
    def part5_tests(self):
        ...
        led_data = self.ii.read_port(0)
        self.comment("Data on LEDs should be max signed (0x77), and found to be {d:#X}".format(d=led_data))
        if led_data != 0x77:
